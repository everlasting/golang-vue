	...
	api.Use(rest.DefaultDevStack...)
   	...
   	api.Use(&rest.CorsMiddleware{
        	...
        	OriginValidator: func(origin string, request *rest.Request) bool {
            		return origin == "http://10.99.70.98"
        	},
        	AllowedMethods: []string{"GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"},
        	AllowedHeaders: []string{
        	    	"Accept", "Content-Type", "X-Custom-Header", "Origin", "Access-Control-Allow-Origin", "x-requested-with", "authorization", "x-csrftoken"},
...
func (i *Impl) InitDB() {
	var err error
	i.DB, err = gorm.Open("mysql", "gorm:gorm@(127.0.0.1)/gorm?charset=utf8&parseTime=True")
	if err != nil {
